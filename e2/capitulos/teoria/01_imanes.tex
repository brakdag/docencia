...
\shade[ball color=red] (0,0) circle (0.3cm);
\draw[black, thick] (0,0) circle (0.3cm);
\fill[black] (0,0) circle (0.1cm);
\shade[ball color=blue] (0,-1) circle (0.3cm);
\draw[black, thick] (0,-1) circle (0.3cm);
\fill[black] (0,-1) circle (0.1cm);
\end{tikzpicture}

\begin{tikzpicture}[scale=1.5]
  \draw[-o] (-1,0) -- (4,0) node[right] {$x$};
  \draw[-o] (0,-1) -- (0,4) node[above] {$y$};
  \draw[domain=-0.2:3.2,smooth,variable=\x,blue] plot ({\x},{sin(\x r)}) node[right] {$y=\sin x$};
\end{tikzpicture}
